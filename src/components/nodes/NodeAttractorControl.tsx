...
interface NodeAttractorControlProps {
...
export function NodeAttractorControl({
  containerRef,
  canvasRef,
  previewZoom,
  panOffset,
}: NodeAttractorControlProps) {
  ...
  // Check if the selected node is an attractor
  const isAttractor = selectedNode?.type === 'attractor';

  // Get attractor values (use defaults if not available)
  const attractorX = isAttractor ? ((selectedNode.params.attractorX as number) ?? 50) : 50;
  const attractorY = isAttractor ? ((selectedNode.params.attractorY as number) ?? 50) : 50;
  const radius = isAttractor ? ((selectedNode.params.radius as number) ?? 100) : 100;
  ...
  useEffect(() => {
    ...
    // Only run animation loop when there's an attractor to show
    if (isAttractor) {
      animationId = requestAnimationFrame(animateUpdate);
    }
  ...
  }, [canvasRef, containerRef, previewZoom, panOffset, isAttractor]);
  ...
  const centerCanvasX = (attractorX / 100) * canvas.width;
  const centerCanvasY = (attractorY / 100) * canvas.height;
  const centerScreenX = centerCanvasX * scaleX;
  const centerScreenY = centerCanvasY * scaleY;

  // Calculate radius in screen coords
  const radiusScreen = radius * scaleX;
  ...
  useEffect(() => {
    // Early exit if not dragging or no attractor selected
    if (!isDraggingCenter && !isDraggingRadius) return;
    if (!isAttractor || !selectedNode) return;
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (isDraggingCenter) {
        ...
        updateNodeParams(selectedNode.id, {
          attractorX: Math.round(percentX),
          attractorY: Math.round(percentY),
        });
        ...
        // Convert to mm
        const radiusMm = Math.max(10, Math.min(500, Math.round(screenDist / scaleX)));
        updateNodeParams(selectedNode.id, {
          radius: radiusMm,
        });
      }
  ...
  if (!isAttractor || !overlayPosition || overlayPosition.width === 0) {
    return null;
  }
